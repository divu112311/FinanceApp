...
const LearningCenter: React.FC<LearningCenterProps> = ({ user, xp, onXPUpdate }) => {
  ...
  const handleStartModule = async (moduleId: string) => {
    ...
    // Simulate completion for demo (in real app, this would be based on actual content consumption)
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {module.progress?.status === 'completed' ? 'Completed' : 
                       module.progress?.status === 'in_progress' ? 'Continue' : 'Start Learning'}
                    </motion.button>
                  </div>
                </motion.div>
              );
                  ...
                  {isLocked ? (
                    ...
                    >
                      {module.progress?.status === 'completed' ? 'Done' : 
                       module.progress?.status === 'in_progress' ? 'Continue' : 'Start'}
                    </motion.button>
                  )}
                </div>
              </motion.div>
